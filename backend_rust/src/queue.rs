...
mod tests {
    ...
    fn test_enqueue_then_dequeue_one() {
        let q = Queue::new(3);
        assert!(q.enqueue(Frame::new(CameraId::Camera1, 1, SystemTime::now())).is_ok());
        
        let frame = q.dequeue().unwrap().expect("Dequeuing failed and should have succeeded.");

        assert_eq!(frame.camera_id(), CameraId::Camera1);
    ...
    fn test_drop_oldest_when_full() {
        let q = Queue::new(3);
        assert!(q.enqueue(Frame::new(CameraId::Camera2, 1, SystemTime::now())).is_ok());
        assert!(q.enqueue(Frame::new(CameraId::Camera2, 2, SystemTime::now())).is_ok());
        assert!(q.enqueue(Frame::new(CameraId::Camera2, 3, SystemTime::now())).is_ok());
        assert!(q.enqueue(Frame::new(CameraId::Camera2, 4, SystemTime::now())).is_ok());
        ...
        assert_eq!(frame1.frame_id(), 2);
        assert_eq!(frame2.frame_id(), 3);
        assert_eq!(frame3.frame_id(), 4);
    }

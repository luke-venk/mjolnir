...
pub fn main() {
    ...
    if camera_ids.is_empty() {
        println!("No cameras were found on the network to record with. Please try recording/streaming again...");
        return;
    }
    ...
    for camera_id in camera_ids {
        ...
        let camera_ingest_config: CameraIngestConfig = CameraIngestConfig::from_record_both_args(camera_id, args.clone());
    ...
    for handle in handles {
        handle.join().expect("Error: Failed to join recording thread.");
    }
